...
communicate only through one integration event carried over RabbitMQ.
...
connection, so a `200` from either one is a real signal that the whole chain
(DB + bus) is reachable, not just that the process is up.
...
client, etc.) and run the requests top to bottom — it's built as one linear
walkthrough: create a piece, register a session against it, watch the piece
update, hit the failure cases (409 on premature mastery, 404s, 400 on bad
input).
...
**Gotcha worth knowing before you hand-write a request body:** no
...
string, parsed case-insensitively (`"Polishing"`, `"Mastered"`, ...).
Response bodies always come back as strings (`piece.Status.ToString()`), so
what you read is never what you write for those fields — a real asymmetry,
not a typo.
...
- `grep -rn "Status ==" src` outside the two Domain projects returns exactly
  ...
  filter on `GET /pieces` (T29) — it filters rows, it does not decide
  anything. The rule RNF05 protects — "no code outside `Piece.cs` decides
  whether a status transition is legal" — is still intact: `ChangeStatus`
  and `RegisterPractice` on `Piece.cs` are the only places that branch on
  what a status transition means. Judgment call, noted here so it doesn't
...
**One item requires a live run, not code-reading, to fully believe:** the
Repertoire-stopped-then-restarted recovery. Do it yourself:
...
Retry + `_error` queue (RNF08, T63): to see it, publish a message the
consumer can't handle (e.g. temporarily throw inside `Consume`) — MassTransit
retries with an increasing interval (1s, then 2s, per
`retryConfigurator.Incremental(3, ...)` in
[Program.cs](src/Repertoire/Cadence.Repertoire.Api/Program.cs)), then the
...
publishes what happened, Repertoire reacts by folding it into the piece.
...
least one side, to keep one context's model changes from leaking into the
...
other dependents). Conformist is the honest choice, not a shortcut taken
under time pressure.
...
event-carried state transfer, deliberately: Practice doesn't need composer,
key, difficulty, or status to render a history list, and a session record
describes what you practiced *at the time*, so title drift after a rename is
accepted, not a bug.

---

**Why does `Cadence.Repertoire.Domain` not reference `Cadence.Contracts`?
What would break, concretely, if it did?**
...
`Cadence.Repertoire.Api`, not `.Domain`). If Domain referenced Contracts,
two things break: first, the dependency rule Api → Domain, Infrastructure,
Contracts / Domain → nothing becomes false, so `T23`'s zero-package-reference
check (and the architecture test it encodes) fails immediately, and it
becomes physically possible to write `PracticeFocus` or
`PracticeSessionRegistered` straight into aggregate logic — the domain-local
`PracticeFocus` enum duplicated in `Practice.Domain` (T45) exists specifically
to prevent that coupling. Second, Contracts is a public wire contract shared
by both services; changing it to suit one domain's internal modeling needs
would ripple into the other service's deserialization, which is exactly the
kind of coupling bounded contexts are supposed to prevent.

**`PracticeRecord.Register` returns a new value object instead of mutating
the existing one. What does immutability buy here that a mutable class would
not?**

Three things, concretely. First, thread/concurrency safety for free: nothing
can observe a `PracticeRecord` in a half-updated state (minutes summed but
count not yet incremented), because there's no such state — the transition
is atomic by construction, not by convention. Second, it makes
...
domain model's actual semantics. Third — the real payoff — it makes the
arithmetic in `Register` pure and trivially unit-testable in isolation: given
inputs, assert on the returned value, no setup/teardown of mutable state, no
ordering-dependent test pollution. A mutable class buys nothing here in
exchange, since nothing about practice history is ever supposed to be
partially visible or reverted.

**The rule "a piece cannot be mastered without practice" lives on the
aggregate. Name two other places it could plausibly have been put, and say
...
exists in two places the moment `RegisterPractice`'s automatic-promotion path
also needs to reason about masterability, and the two copies can silently
diverge — exactly the anemic-domain-model failure mode where "the real
business logic" leaks into a layer that's supposed to be plumbing. Any second
caller of `ChangeStatus` (a future admin tool, a batch job, a test harness)
gets an *unenforced* rule unless it remembers to re-check.

*In a database constraint* (e.g. a `CHECK` on the `Status`/`SessionCount`
columns): the rule becomes invisible to anyone reading C#, fires only at
`SaveChanges` time as an opaque SQL error instead of a `Result` with a
...
**If a session is registered and RabbitMQ is briefly unreachable, describe
exactly what the user sees and what state the system is left in. What would
the outbox pattern change about that answer?**

The user sees success: `PracticeDbContext.SaveChangesAsync` already
committed the `PracticeSession` row before the publish is attempted, so
...
unreachable" lands: if MassTransit's bus is still starting up or reconnecting
when `IPublishEndpoint.Publish` is called, the call can throw (the endpoint
isn't ready) or block briefly waiting for a channel, but either way there's
no retry wrapping *that specific call* the way there is on the consumer side
— it either eventually succeeds once the connection recovers, or the request
fails outright/the event is silently dropped, and there is nothing recorded
anywhere that says "this session's event was supposed to be published and
wasn't." The system is left with a `PracticeSession` in `practice.db` that
Repertoire may never learn about — the exact gap documented in
...
publishes, retrying indefinitely against RabbitMQ until it succeeds, deleting
the outbox row only after a confirmed publish. The user-visible behavior
becomes strictly better in the failure case (the event is guaranteed to
eventually reach Repertoire, no silent loss) at the cost of eventual — not
immediate — consistency between "session saved" and "event published," which
this system already tolerates everywhere else (RabbitMQ delivery was never
synchronous with the HTTP response to begin with).
